...
sp500_asset = asset_def("S&P 500", mkt=1, smb=-0.16)

iusv_ff5_asset = asset_def(
    "IUSV (value)",
    ter=0.04,
    mkt=0.97,
    smb=0.03,
...
ijs_ff5_asset = asset_def(
    "IJS (scv)", ter=0.25, mkt=1.04, smb=0.86, hml=0.21, rmw=0.19, cma=0.09, mom=-0.05
)
dfsvx_ff5_asset = asset_def(
    "DFSVX (scv)", ter=0.4, mkt=1.03, smb=0.87, hml=0.40, rmw=0.14, cma=0.08, mom=-0.08
)
vfmf_ff5_asset = asset_def(
    "VFMF", ter=0.18, mkt=0.98, smb=0.47, hml=0.36, rmw=0.16, cma=0.12, mom=0.19
)

itt_asset = asset_def("ITT", ter=0, itt=1)
ltt_asset = asset_def("LTT", ter=0, ltt=1)
tmf_asset = asset_def("TMF", ter=1.1, ltt=3)
upro_asset = asset_def("UPRO", ter=2.0, mkt=3, smb=-0.16 * 3)

BOUNDS = (0, 1)
CASH_BOUNDS = (0, 1)
SHORTING_COST = 0.5 / 100
X_AXIS = (0, 0.50)  # annualized standard deviation
Y_AXIS = (0, 0.12)  # annualized return
...
assets = np.array([asset_to_weight_array(asset) for asset in chosen_assets])
...
def find_weights(expected_std):
    ...
    constraints = [
        (
            {"type": "eq", "fun": lambda w: sum(model.extract_weights(w)) - 1.0}
        ),  # weights must sum to 1
        (
            {"type": "eq", "fun": lambda w: model.mean_and_std(w)[1] - expected_std}
        ),  # std must be equal to expected_std
    ]
...
for expected_std in np.linspace(X_AXIS[0], X_AXIS[1], 200):
    vars = find_weights(expected_std / 12**0.5)
    ...
    print("mean: {:.2%}, std: {:.2%}".format(mean * 12, std * 12**0.5))
    Y.append(mean * 12)
    X.append(std * 12**0.5)
...
def isoelastic_utility(X, Y, gamma):
    return np.array(Y) - 0.5 * np.power(X, 2) * gamma
...
ax1.text(
    0.30,
    0.005,
    "γ = 0 is the efficient frontier\nγ = 1 maximizes geometric growth,\nalso known as the kelly criterion",
    bbox=dict(boxstyle="square", ec=(0.6, 0.6, 0.6), fc=(1, 1, 1)),
)
